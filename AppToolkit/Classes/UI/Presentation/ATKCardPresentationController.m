//
//  ATKCardPresentationController.m
//  AppToolkitRemoteUIBuilding
//
...
@interface ATKCardPresentationController ()
...
@implementation ATKCardPresentationController
...
- (void)presentationTransitionWillBegin {
    ...
    if ([self.presentedViewController isKindOfClass:[ATKViewController class]]) {
        // cardView is often the same as view, but in some cases (complex
        // shadow + rounded corner situation), it's not
        roundView = ((ATKViewController *)self.presentedViewController).cardView;
    }
    ...
    // set (usually through a AppToolkit remote UI storyboard :))
...
- (CGRect)frameOfPresentedViewInContainerView {
    if (self.presentedViewController.traitCollection.verticalSizeClass == UIUserInterfaceSizeClassCompact || [self shouldPresentInFullscreen]) {
        ...
        ATKViewController *presentedATKVC = nil;
        if ([self.presentedViewController isKindOfClass:[ATKViewController class]]) {
            presentedATKVC = (ATKViewController *)self.presentedViewController;
        }
        if (presentedATKVC.hasMeasureableSize) {
...
- (CGSize)measuredSizeOfView:(UIView *)view atWidth:(CGFloat)maxWidth
{
    ...
    BOOL useCache = ![ATKCardPresentationController isiOS9AndUp];
...
{
    if ([self.presentedViewController isKindOfClass:[ATKViewController class]]) {
        [((ATKViewController *)self.presentedViewController) finishFlowWithResult:ATKViewControllerFlowResultCancelled userInfo:nil];
    } else {
        // NOTE: This doesn't make any callbacks to ATKUIManager, in case ATKUIManager is presenting this view controller.
