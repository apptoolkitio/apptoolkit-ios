...
@interface ATKViewController (ATKHelperAdditions)

- (void) markFinishedFlowResult:(ATKViewControllerFlowResult)result;

@end

@interface ATKOnboardingViewController () <ATKViewControllerFlowDelegate>
...
@property (strong, nonatomic, nullable) ATKViewController *remoteOnboardingViewController;
...
@implementation ATKOnboardingViewController
...
- (void) loadView
{
    ...
    if (!loadedLaunchImage && !loadedLaunchStoryboard) {
        ATKLogError(@"Neither LaunchImage or Launch screen was loaded");
    }
}
    ...
    if (launchScreenName.length > 0) {
        ...
        if (storyboardPath.length > 0) {
            ...
            @catch (NSException *exception) {
                ...
                ATKLogWarning(@"%@.storyboard not found", launchScreenName);
            }
            @finally {
                //
            }
        ...
        if (!self.launchViewController && nibPath.length > 0) {
            ...
            @catch (NSException *exception) {
                ...
                ATKLogWarning(@"%@.nib not found", launchScreenName);
            }
            @finally {
                //
            }
...
- (void) viewDidAppear:(BOOL)animated
{
    ...
    // window during the hand-off back the app's UI (see ATKUIManager).
    // So check if we already have finished and do nothing, if so.
    if (self.finishedFlowResult != ATKViewControllerFlowResultNotSet) {
        return;
    }
...
- (void) onMaxLoadingTimeoutTimerFired:(NSTimer *)timer
{
    ...
    if (!self.remoteOnboardingViewController) {
        ATKLogError(@"Actual onboarding UI failed to load from remote after %.1f seconds, failing...", self.maxWaitTimeInterval);
        [self finishOnboardingWithResult:ATKViewControllerFlowResultFailed];
    }
}
...
- (void) setActualOnboardingUI:(ATKViewController *)actualOnboardingUI
{
    if (self.finishedFlowResult != ATKViewControllerFlowResultNotSet || actualOnboardingUI == nil) {
        return;
    }
...
- (void) finishFlowWithResult:(ATKViewControllerFlowResult)result userInfo:(NSDictionary *)userInfo
{
    ...
    if (self.dismissalHandler) {
        ATKBundleInfo *actualOnboardingBundleInfo = self.remoteOnboardingViewController.bundleInfo;
