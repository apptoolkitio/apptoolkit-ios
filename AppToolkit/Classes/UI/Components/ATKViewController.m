...
#import "ATKPopCustomSegue.h"
#import "UIView+ATKAdditions.h"

@interface ATKViewController ()
...
- (void)viewDidLoad {
    ...
    if (self.cardView.atk_cornerRadius != self.viewCornerRadius) {
        self.cardView.atk_cornerRadius = self.viewCornerRadius;
    }
...
- (IBAction)finishFlowWithSuccess:(UIStoryboardSegue *)segue
{
    ...
    [self finishFlowWithResult:ATKViewControllerFlowResultCompleted userInfo:nil];
}
...
{
    // Determine the actual view controller to finish with
    [self finishFlowWithResult:ATKViewControllerFlowResultCancelled userInfo:nil];
}
...
{
    // Determine the actual view controller to finish with
    [self finishFlowWithResult:ATKViewControllerFlowResultFailed userInfo:nil];
}
...
- (UIStoryboardSegue *) segueForUnwindingToViewController:(UIViewController *)toViewController fromViewController:(UIViewController *)fromViewController identifier:(NSString *)identifier
{
    ...
    if ([fromViewController isKindOfClass:[ATKViewController class]]) {
        customUnwindSegueName = ((ATKViewController *)fromViewController).unwindSegueClassName;
    }

    if ([customUnwindSegueName isEqualToString:@"ATKPopCustomSegue"]) {
        return [[ATKPopCustomSegue alloc] initWithIdentifier:identifier source:fromViewController destination:toViewController];
    } else {
...
/// This will finish the flow with the earliest ATKViewController in the stack, that
/// has a non-nil flowDelegate
- (void) finishFlowWithResult:(ATKViewControllerFlowResult)result userInfo:(nullable NSDictionary *)userInfo
{
    UIViewController *viewController = self;
    while (viewController != nil) {
        ATKViewController *lkvc = nil;
        if ([viewController isKindOfClass:[ATKViewController class]]) {
            lkvc = (ATKViewController *)viewController;
        }
        if (lkvc.flowDelegate != nil) {
            [lkvc.flowDelegate appToolkitController:lkvc didFinishWithResult:result userInfo:userInfo];
            break;
        } else {
...
- (void) markFinishedFlowResult:(ATKViewControllerFlowResult)result
{
...
- (void)setViewCornerRadius:(CGFloat)viewCornerRadius
{
    ...
    if (self.isViewLoaded) {
        self.cardView.atk_cornerRadius = _viewCornerRadius;
    }
}
...
NSString * NSStringFromViewControllerFlowResult(ATKViewControllerFlowResult result)
{
    switch (result) {
        case ATKViewControllerFlowResultNotSet:
            return @"not-set";
        case ATKViewControllerFlowResultCompleted:
            return @"completed";
        case ATKViewControllerFlowResultCancelled:
            return @"cancelled";
        case ATKViewControllerFlowResultFailed:
            return @"failed";
        default:
