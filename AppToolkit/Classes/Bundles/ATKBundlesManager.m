...
static ATKBundlesManager *_sharedInstance;

NSString *const ATKBundlesManagerDidFinishRetrievingBundlesManifest = @"ATKBundlesManagerDidFinishRetrievingBundlesManifest";
NSString *const ATKBundlesManagerDidFinishDownloadingRemoteBundles = @"ATKBundlesManagerDidFinishDownloadingRemoteBundles";

// Add category that reveals our (private) ATKBundleInfo method to upgrade resource version to newest
// (once we have retrieved the remote manifest and determined that this is, in fact, the newest available
@interface ATKBundleInfo (UpdatingResourceVersion)
- (void) markResourceVersionAsNewest;
@end

@interface ATKBundlesManager ()

@property (strong, nonatomic) NSMutableDictionary<NSString *, ATKBundleInfo *> *remoteBundleMap;
@property (strong, nonatomic) NSMutableDictionary<NSString *, ATKBundleInfo *> *localBundleMap;
...
// This is stored on our [application support]/apptoolkit/bundles folder
...
@implementation ATKBundlesManager
...
- (NSDictionary *)stateDictionary
{
    ...
    lastAppUsageInfo[APP_USAGE_VERSION_KEY] = [ATKAPIClient appBundleVersion];
    lastAppUsageInfo[APP_USAGE_BUILD_KEY] = [ATKAPIClient appBuildNumber];
...
- (void) updateServerBundlesUpdatedTimeWithTime:(NSDate *)bundlesUpdatedTime
{
    ...
    if (localTimestamp == serverTimestamp) {
        if (self.debugMode) {
            if (self.verboseLogging) {
                ATKLog(@"Manifest is up-to-date, and remoteBundlesDownloaded = %d", self.remoteBundlesDownloaded);
            }
        }
        ...
        if (!previouslyRetrieved) {
            ...
            for (NSString *key in self.localBundleMap) {
                ATKBundleInfo *info = self.localBundleMap[key];
    ...
    // called. It essentially "self-corrects", eventually (Assuming ATK is still making
    // track calls).
    if (!self.retrievingRemoteBundles) {
        ...
        if (self.debugMode) {
            if (self.verboseLogging) {
                if ([self.localBundlesFolderUpdatedTime isEqualToDate:[NSDate distantPast]]) {
                    ATKLog(@"RETRIEVING Manifest+Bundles because: Local bundle time is not available, assuming no bundles");
                } else {
                    ATKLog(@"RETRIEVING Manifest+Bundles because: Local bundle time (%@) != server time (%@)",
        ...
        [self retrieveAndCacheAvailableRemoteBundlesWithAssociatedServerTimestamp:bundlesUpdatedTime completion:^(NSError *error) {
            if (error) {
                ATKLogWarning(@"Received error downloading and caching remote bundles: %@", error);
            } else {
                ATKLog(@"Remote bundles downloaded and cached.");
            }
        }];
    }
}
...
{
  NSURL *bundlesURL = [ATKBundlesManager bundlesCacheDirectoryURLCreateIfNeeded:YES];
...
+ (nullable NSDate *)updateTimeInLocalBundlesFolder
{
    ...
    NSData *updateTimeStringData = [ATKBundlesManager dataFromLocalBundlesFileNamed:@"ATKWuzHere.txt"];
...
+ (void)setUpdateTimeInLocalBundlesFolder:(NSDate *)updateTime
{
    ...
    NSURL *bundlesURL = [ATKBundlesManager bundlesCacheDirectoryURLCreateIfNeeded:YES];
    NSURL *bundlesCanaryFileURL = [bundlesURL URLByAppendingPathComponent:@"ATKWuzHere.txt"];
...
+ (NSURL *)bundlesCacheDirectoryURLCreateIfNeeded:(BOOL)createIfNeeded
{
    ...
    NSString *bundlesCacheDir = [appToolkitDir stringByAppendingPathComponent:@"bundles"];
    ...
    if (createIfNeeded && ![fileManager fileExistsAtPath:bundlesCacheDir isDirectory:&isDirectory] && isDirectory) {
        ...
        if (folderCreateError != nil) {
            ...
            ATKLogError(@"Could not create bundles cache folder at %@, error: %@", bundlesCacheDirUrl.path, folderCreateError);
            return nil;
        }
        ...
        if (excludeFromiCloudError != nil) {
            ...
            ATKLogWarning(@"Was not able to exclude bundles cache folder at %@ from iCloud Backups. Error: %@", bundlesCacheDirUrl.path, excludeFromiCloudError);
        }
    }
...
{
    NSURL *bundlesCacheDirUrl = [ATKBundlesManager bundlesCacheDirectoryURLCreateIfNeeded:NO];
    if (bundlesCacheDirUrl) {
        ...
        if (deleteBundlesDirError != nil && !([deleteBundlesDirError.domain isEqualToString:@"NSCocoaErrorDomain"] && deleteBundlesDirError.code == NSFileNoSuchFileError)) {
            ATKLogWarning(@"Unable to delete bundles cache directory: %@", deleteBundlesDirError);
        }
    }
}
...
- (void)deleteBundlesCacheDirectoryIfNeeded
{
    ...
    if (self.appUsageInfo) {
        NSString *currentAppVersion = [ATKAPIClient appBundleVersion];
        NSString *currentAppBuild = [ATKAPIClient appBuildNumber];
        ...
        // at or before ATK instantiation, otherwise we can't use ATKLog
...
- (void)rebuildLocalBundlesMap
{
    ...
    if (LOAD_PREPACKAGED_BUNDLES) {
        ...
        NSString *mainBundleBaseSubdirectory = @"AppToolkitRemoteResources";
        ...
        for (NSURL *urlWithBundleName in urlsWithBundleNames) {
            ...
            self.localBundleMap[bundleName] = [[ATKBundleInfo alloc] initWithName:bundleName
                                                                         version:version
                                                                             url:bundleUrl
                                                                      createTime:createTime
                                                                 resourceVersion:ATKResourceVersionPrepackaged];
        }
    }
    ...
    if (LOAD_CACHED_BUNDLES) {
        ...
        for (NSURL *fileUrl in fileUrls) {
            ...
            ATKBundleInfo *info = [[ATKBundleInfo alloc] initWithName:name
                                                            version:version
                                                                url:localCacheUrl
                                                         createTime:mostRecentCreateTime
                                                    resourceVersion:ATKResourceVersionLocalCache];
    ...
    NSDate *localUpdateTime = [ATKBundlesManager updateTimeInLocalBundlesFolder];
...
- (void) retrieveRemoteBundlesManifestWithCompletion:(void (^)(NSError *error))completion
{
    ...
    void (^finishWithError)(NSError *error) = ^(NSError *error) {
        dispatch_async(dispatch_get_main_queue(), ^{
...
#if DEBUG
            if (self.debugMode) {
                ATKLog(@"ATKBundlesManager: Finished retrieving remote bundle manifest.");
                if (self.verboseLogging) {
                    NSTimeInterval timeTaken = -[startDate timeIntervalSinceNow];
                    ATKLog(@"ATKBundlesManager: Took %.2f seconds to retrieve bundles manifest", timeTaken);
                }
            }
#endif
            ...
            [[NSNotificationCenter defaultCenter] postNotificationName:ATKBundlesManagerDidFinishRetrievingBundlesManifest
                                                                object:self];
        });
    };
    if (self.debugMode) {
        ATKLog(@"ATKBundlesManager: Retrieving remote bundle manifest...");
    }
    self.retrievingRemoteBundlesManifest = YES;
    __weak ATKBundlesManager *_weakSelf = self;
    [self.apiClient retrieveBundlesManifestWithSuccessBlock:^(NSArray *bundleInfos) {
        ...
        for (ATKBundleInfo *bundleInfo in bundleInfos) {
            // Check our local bundle map, and
            // if we have the same version,
            // mark our local version as "newest"
            ATKBundleInfo *localBundleInfo = self.localBundleMap[bundleInfo.name];
...
- (void)retrieveAndCacheAvailableRemoteBundlesWithAssociatedServerTimestamp:(NSDate *)serverTimestamp completion:(void (^)(NSError *error))completion
{
    ...
    void (^finish)(NSError *) = ^(NSError *error) {
        ...
        if (self.debugMode && self.verboseLogging) {
            NSTimeInterval timeTaken = -[startDate timeIntervalSinceNow];
            ATKLog(@"ATKBundlesManager: Took %.2f seconds to retrieve and cache remote bundles", timeTaken);
        }
#endif
        if (completion) {
            completion(error);
...
- (void)loadBundleWithId:(NSString *)bundleId completion:(ATKRemoteBundleLoadHandler)completion
{
    ...
    if (self.latestRemoteBundlesManifestRetrieved) {
        ...
        ATKBundleInfo *remotelyAvailableBundleInfo = [self remoteBundleInfoWithName:bundleId];
        if (!remotelyAvailableBundleInfo) {
            if (self.debugMode && self.verboseLogging) {
                ATKLog(@"Bundle with id '%@' not found in remote manifest, so failing early.", bundleId);
            }
            if (completion) {
    ...
    if (!self.hasNewestRemoteBundles || self.retrievingRemoteBundles) {
        if (self.debugMode && self.verboseLogging) {
            ATKLog(@"Waiting for bundles to finish loading before loading bundle '%@'...", bundleId);
        }
        if (completion) {
...
{
    ATKBundleInfo *bundleInfo = [self localBundleInfoWithName:bundleId];
    if (!bundleInfo || !bundleInfo.url) {
        *error = [self bundleInfoNotFoundErrorForId:bundleId];
        if (self.debugMode && self.verboseLogging) {
            ATKLogError(@"Bundle '%@' not found in local bundles manifest", bundleId);
        }
        return nil;
    }
    ...
    if (!bundle) {
        *error = [self bundleLoadErrorForUrl:bundleInfo.url];
        if (self.debugMode && self.verboseLogging) {
            ATKLogError(@"Couldn't load an NSBundle within ATK bundle '%@'. Error: %@", bundleId, *error);
        }
        return nil;
    }
...
- (void) notifyAnyPendingBundleLoadHandlers
{
    for (NSString *bundleId in self.pendingRemoteBundleLoadHandlers) {
        ...
        for (NSInteger i = 0; i < loadHandlers.count; i++) {
            ATKRemoteBundleLoadHandler loadHandler = loadHandlers[i];
...
- (NSError *)bundleInfoNotFoundErrorForId:(NSString *)bundleId
{
    ...
    return [[NSError alloc] initWithDomain:@"ATKBundlesManagerError"
                                      code:404
...
{
    NSFileManager *fileManager = [NSFileManager defaultManager];
    NSURL *bundleVersionsDir = [[ATKBundlesManager bundlesCacheDirectoryURLCreateIfNeeded:NO] URLByAppendingPathComponent:name];
    ...
    BOOL (^deleteBundleFolder)(void) = ^BOOL {
        ...
        if (folderDeleteError == nil) {
            ATKLog(@"Deleted local folder for bundle '%@'", name);
        }
    ...
    for (NSURL *versionFolderUrl in versionFolderUrls) {
        ...
        if (versionFolderDeleteError != nil) {
            ATKLogWarning(@"Unable to delete version: %@ of bundle '%@': %@",
    ...
    if (numDeleted == versionFolderUrls.count) {
        ...
        ATKLog(@"Deleted %lu old versions of bundle '%@'", numDeleted, name);
...
- (void)deleteLocalBundleInfo:(ATKBundleInfo *)bundleInfo
{
    NSFileManager *fileManager = [NSFileManager defaultManager];
...
- (NSMutableArray<ATKBundleInfo *> *) remoteBundleInfosNeedingDownloadForceRetrieve:(BOOL)forceRetrieve
{
    NSMutableArray<ATKBundleInfo *> *infosNeedingDownload = [NSMutableArray arrayWithCapacity:self.remoteBundleMap.count];
    // Make a list of infos that need to be downloaded
    for (NSString *name in self.remoteBundleMap) {
        ATKBundleInfo *remoteInfo = self.remoteBundleMap[name];
        ATKBundleInfo *localInfo = self.localBundleMap[name];
...
- (void) downloadRemoteBundlesForceRetrieve:(BOOL)forceRetrieve associatedServerTimestamp:(NSDate *)serverTimestamp completion:(void (^)(NSError *error))completion
{
    ...
    NSMutableArray<ATKBundleInfo *> *infosNeedingDownload = [self remoteBundleInfosNeedingDownloadForceRetrieve:forceRetrieve];
    ...
    void (^onDownloadsFinished)(unsigned long long downloadSize, NSError *error) = ^(unsigned long long downloadSize, NSError *error) {
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            if (STORE_SERVER_BUNDLE_UPDATE_TIME) {
                [ATKBundlesManager setUpdateTimeInLocalBundlesFolder:serverTimestamp];
                _weakSelf.localBundlesFolderUpdatedTime = serverTimestamp;
            }
            if (self.debugMode && infosNeedingDownload.count > 0) {
                ATKLog(@"ATKBundlesManager: Finished downloading remote bundles.");
            }
#if DEBUG
            if (self.debugMode && self.verboseLogging) {
                NSTimeInterval timeTaken = -[startDate timeIntervalSinceNow];
                unsigned long long kilobytes = downloadSize / (unsigned long long) 1024;
                ATKLog(@"ATKBundlesManager: Took %.2f seconds to download %lluKB remote bundles", timeTaken, kilobytes);
            }
#endif
            [self notifyAnyPendingBundleLoadHandlers];
            if (completion) {
            ...
            [[NSNotificationCenter defaultCenter] postNotificationName:ATKBundlesManagerDidFinishDownloadingRemoteBundles
                                                                object:_weakSelf];
        });
    };
    ...
    if (infosNeedingDownload.count > 0) {
        if (self.debugMode) {
            ATKLog(@"ATKBundlesManager: Downloading %lu remote bundles...", (unsigned long)infosNeedingDownload.count);
        }
        self.downloadingRemoteBundles = YES;
        for (ATKBundleInfo *info in infosNeedingDownload) {
            if (self.debugMode && self.verboseLogging) {
                ATKBundleInfo *localInfo = _weakSelf.localBundleMap[info.name];
                NSString *newOrUpdating = @"new";
                if (localInfo != nil) {
                    newOrUpdating = @"update";
                }
                ATKLog(@"ATKBundlesManager: Downloading %@ version %@ (%@)...", info.name, info.version, newOrUpdating);
            }
            [_weakSelf downloadBundleFromInfo:info deleteOtherVersions:YES completion:^(ATKBundleInfo *savedInfo, unsigned long long downloadSize, NSError *error) {
                numItemsToDownload--;
        ...
        if (self.debugMode) {
            ATKLog(@"ATKBundlesManager: No need to download any remote bundles.");
        }
...
- (void) downloadBundleFromInfo:(ATKBundleInfo *)info deleteOtherVersions:(BOOL)deleteOtherVersions completion:(void(^)(ATKBundleInfo *savedInfo, unsigned long long downloadSize, NSError *error))completion
{
    NSURL *remoteUICacheDirUrl = [ATKBundlesManager bundlesCacheDirectoryURLCreateIfNeeded:YES];
    ...
    [self saveDataFromRemoteUrl:info.url toDirectoryUrl:localCacheParentUrl completion:^(NSURL *savedFileUrl, unsigned long long downloadSize, NSError *error) {
        if (completion) {
            dispatch_async(dispatch_get_main_queue(), ^{
                ATKBundleInfo *savedInfo = nil;
                if (savedFileUrl != nil) {
                    savedInfo = [[ATKBundleInfo alloc] initWithName:info.name
                                                           version:info.version
                                                               url:savedFileUrl
                                                        createTime:info.createTime
                                                   resourceVersion:ATKResourceVersionNewest];
        ...
        if (error) {
            ...
            if ([fileManager fileExistsAtPath:directoryUrl.path]) {
                ...
                if (deleteExistingFileError != nil) {
                    ATKLogError(@"Couldn't delete existing item at %@ in order to download a new copy. Error: %@", deleteExistingFileError);
                    if (completion) {
