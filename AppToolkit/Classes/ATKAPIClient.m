...
NSString* const ATKAPIFailedAuthenticationChallenge = @"ATKAPIFailedAuthenticationChallenge";

static NSString* const API_ERROR_DOMAIN = @"AppToolkitAPI";

#define ATK_DEBUG_LOG_REQUESTS 0
#define ATK_DEBUG_RETRIEVE_LATEST_HOSTED_BUNDLES 0
...
- (instancetype) init
{
    self = [super init];
    if (self) {
        _cachedBundleIdentifier = [ATKAPIClient appBundleIdentifier];
        _cachedBundleVersion = [ATKAPIClient appBundleVersion];;
        _cachedBuildNumber = [ATKAPIClient appBuildNumber];
        _cachedOSVersion = [NSString stringWithFormat:@"iOS %@", [ATKAPIClient softwareVersion]];
        _cachedHardwareModel = [ATKAPIClient hardwareModel];
        ...
        _urlSessionConfiguration.HTTPAdditionalHeaders = @{@"User-Agent" : [ATKAPIClient userAgentString]};

        _urlSessionQueue = [[NSOperationQueue alloc] init];
        _urlSessionQueue.name = @"AppToolkit SDK API Queue";
        ...
        _urlSession.sessionDescription = @"AppToolkit SDK URL Session";
    }
    return self;
}
              ...
              errorBlock:(void(^)(NSError *error))errorBlock
{
    ...
    params[@"screen"] = [ATKAPIClient currentWindowInfo];
    params[@"system_locale"] = _cachedLocaleIdentifier;
    params[@"app_localization"] = _cachedAppLocalization;
#if DEBUG
    // Notify ATK servers when the app is running in debug mode
                                      ...
                                      errorBlock:(void(^)(NSError *error))errorBlock
{
...
#if DEBUG
#if ATK_DEBUG_RETRIEVE_LATEST_HOSTED_BUNDLES
    ...
    [self objectFromPath:path method:@"GET" params:params successBlock:^(NSDictionary *responseDict) {
        ...
        for (NSDictionary *rawBundleInfo in rawBundleInfos) {
            ATKBundleInfo *bundleInfo = [[ATKBundleInfo alloc] initWithAPIDictionary:rawBundleInfo];
           ...
           failureBlock:(void(^)(NSError *))failureBlock
{
    ...
    if ([method isEqualToString:@"GET"] && params != nil) {
        NSString *formEncodedParams = [params atk_toFormEncodedString];
...
#if ATK_DEBUG_LOG_REQUESTS
    ATKLog(@"API request: %@ %@", method, urlString);
#endif
    ...
    if ([method isEqualToString:@"POST"] && params != nil) {
        ...
        if (!JSONparams) {
            ...
            body = [[params atk_toFormEncodedString] dataUsingEncoding:NSUTF8StringEncoding];
        } else {
            ...
            if (serializationError != nil) {
                if (self.verboseLogging) {
                    ATKLogError(@"Could not serialize JSON: %@", serializationError);
                }
            }
        }
#if ATK_DEBUG_LOG_REQUESTS
        ATKLog(@"  post body: %@", [[NSString alloc] initWithData:body encoding:NSUTF8StringEncoding]);
#endif
    ...
    void (^completionHandler)() = ^(NSData *data, NSURLResponse *response, NSError *error) {
        ...
        if (error == nil && data != nil && isJSON) {
            ...
            if (jsonError != nil) {
                if (self.verboseLogging) {
                    ATKLogError(@"Bad JSON response: %@", [[NSString alloc] initWithData:data encoding:NSUTF8StringEncoding]);
                }
                if (failureBlock) {
        ...
        if (code != 200 || error != nil) {
            if (self.verboseLogging) {
                ATKLogError(@"Oh noes, an error for request to %@: %ld; %@; %@", path, (long)code, [error description], [error userInfo]);
                if (dict) {
                    ATKLogError(@"Here's the data: %@", dict);
                }
            }
            ...
            if ([error.domain isEqualToString:NSURLErrorDomain] &&
                ...
                dispatch_async(dispatch_get_main_queue(), ^{
                    [[NSNotificationCenter defaultCenter] postNotificationName:ATKAPIFailedAuthenticationChallenge
    ...
    dataTask.taskDescription = [NSString stringWithFormat:@"AppToolkit: %@ %@", method, path];
    [dataTask resume];
    if (self.measureUsage) {
...
{
    CGSize windowSize = [ATKUtils currentWindowSize];
...
{
    NSString *userAgent = [NSString stringWithFormat:@"AppToolkit iOS SDK %@", APPTOOLKIT_VERSION];
    return userAgent;
}
