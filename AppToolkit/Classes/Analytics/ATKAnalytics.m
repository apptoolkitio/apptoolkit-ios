...
NSString *const ATKAppUserUpdatedNotificationName = @"ATKAppUserUpdatedNotificationName";
NSString *const ATKPreviousAppUserKey = @"ATKPreviousAppUserKey";
NSString *const ATKCurrentAppUserKey = @"ATKCurrentAppUserKey";
...
@interface ATKAnalytics () <UIGestureRecognizerDelegate>
...
// Current User Info
@property (strong, nonatomic) ATKAppUser *user;
@property (strong, nonatomic) NSDictionary *lastUserDictionary;

@end

@implementation ATKAnalytics
...
- (void) updateReportingScreens:(BOOL)shouldReport
{
    ...
    if (self.verboseLogging) {
        ATKLog(@"Report Screens turned %@ via remote command", (self.shouldReportScreens ? @"on" : @"off"));
    }
}
...
- (void) updateReportingTaps:(BOOL)shouldReport
{
    ...
    if (self.verboseLogging) {
        ATKLog(@"Report Taps turned %@ via remote command", (self.shouldReportTaps ? @"on" : @"off"));
    }
}
...
- (void)inspectCurrentViewController
{
    ...
    if (![className isEqualToString:self.currentViewControllerClassName]) {
        ...
        ATKLog(@"Current View Controller: %@", self.currentViewControllerClassName);
    }
}
...
- (void)markEndOfVisitationForCurrentViewController
{
    ...
    if (self.currentViewControllerClassName.length && self.currentViewControllerStartTimestamp) {
        ...
        ATKLog(@"%@ seen for about %.0fs", self.currentViewControllerClassName, duration);
...
- (void)startDetectingTapsOnWindow
{
    ...
    self.currentWindowSize = [ATKUtils currentWindowSize];
...
#pragma GCC diagnostic pop
        ...
        if (orientation != UIInterfaceOrientationPortrait) {
            double angle = [ATKAnalytics angleForInterfaceOrientation:orientation];
    ...
    if (self.verboseLogging) {
        ATKLog(@"Tapped %@ within %@", NSStringFromCGPoint(touchPoint), NSStringFromCGRect(frame));
    }
...
- (void) updateUserFromDictionary:(NSDictionary *)dictionary reportUpdate:(BOOL)reportUpdate
{
    ...
    if (self.verboseLogging) {
        ATKLog(@"User object is different, notifying");
    }
    ATKAppUser *currentUser = [[ATKAppUser alloc] initWithDictionary:dictionary];
    // TODO(Riz): Figure out what is different and include in change notification
    ATKAppUser *previousUser = self.user;
    self.user = currentUser;
    self.lastUserDictionary = dictionary;
    if (reportUpdate) {
        NSMutableDictionary *userInfo = [NSMutableDictionary dictionaryWithCapacity:2];
        if (previousUser != nil) {
            userInfo[ATKPreviousAppUserKey] = previousUser;
        }
        if (currentUser != nil) {
            userInfo[ATKCurrentAppUserKey] = currentUser;
        }
        [[NSNotificationCenter defaultCenter] postNotificationName:ATKAppUserUpdatedNotificationName
                                                            object:self
                                                          userInfo:userInfo];
    }
}
...
- (CGRect)frameFixedForOrientation:(UIInterfaceOrientation)interfaceOrientation fromFrame:(CGRect)sourceFrame
{
    ...
    if ([[UIViewController class] instancesRespondToSelector:@selector(viewWillTransitionToSize:withTransitionCoordinator:)]) {
        ...
        double angle = [ATKAnalytics angleForInterfaceOrientation:interfaceOrientation];
