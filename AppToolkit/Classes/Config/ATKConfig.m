...
NSString *const ATKConfigUpdatedNotificationName = @"ATKConfigUpdatedNotificationName";
NSString *const ATKConfigOldParametersKey = @"ATKConfigOldParametersKey";
NSString *const ATKConfigNewParametersKey = @"ATKConfigNewParametersKey";
...
// Define delegate internally here (AppToolkit.m will access via private extension)
@property (weak, nonatomic, nullable) id <ATKConfigDelegate> delegate;

@end

@implementation ATKConfig
...
- (NSDictionary *)dictionaryWithoutAppToolkitKeys:(nonnull NSDictionary *)dictionary
{
    ...
    for (NSString *key in dictionary) {
        if ([key hasPrefix:@"io.apptoolkit."]) {
            continue;
        }
...
- (BOOL) updateParameters:(NSDictionary * __nullable)parameters
{
    ...
    if (![parameters isEqualToDictionary:_parameters] || isFirstRefresh) {
        ...
        NSDictionary *strippedOld = [self dictionaryWithoutAppToolkitKeys:oldParameters];
        NSDictionary *strippedNew = [self dictionaryWithoutAppToolkitKeys:parameters];
        BOOL appVisibleConfigsChanged = ![strippedOld isEqualToDictionary:strippedNew];
        if (appVisibleConfigsChanged) {
            [[NSNotificationCenter defaultCenter] postNotificationName:ATKConfigUpdatedNotificationName
                                                                object:self
                                                              userInfo:@{ATKConfigOldParametersKey: strippedOld,
                                                                         ATKConfigNewParametersKey: strippedNew}];
        }
...
- (BOOL) boolForKey:(NSString * __nonnull)key defaultValue:(BOOL)defaultValue
{
    ...
    if ([value isKindOfClass:[NSNumber class]]) {
        ...
        ATKLogWarning(@"ATKConfig returned value for '%@' is %@, not a BOOL. Returning default: %d",
                      key,
...
- (NSInteger) integerForKey:(NSString * __nonnull)key defaultValue:(NSInteger)defaultValue
{
    ...
    if ([value isKindOfClass:[NSNumber class]]) {
        ...
        ATKLogWarning(@"ATKConfig returned value for '%@' is %@, not an NSInteger. Returning default: %ld",
                      key,
                      NSStringFromClass([value class]),
                      (long)defaultValue);
    }
...
- (double) doubleForKey:(NSString * __nonnull)key defaultValue:(double)defaultValue
{
    ...
    if ([value isKindOfClass:[NSNumber class]]) {
        ...
        ATKLogWarning(@"ATKConfig returned value for '%@' is %@, not a double. Returning default: %@",
                      key,
                      NSStringFromClass([value class]),
...
- (nullable NSString *) stringForKey:(NSString * __nonnull)key defaultValue:(nullable NSString *)defaultValue
{
    ...
    if ([value isKindOfClass:[NSString class]]) {
        ...
        ATKLogWarning(@"ATKConfig returned value for '%@' is %@, not an NSString. Returning default: %@",
                      key,
                      NSStringFromClass([value class]),
                      defaultValue);
    }
