...
let cachedBundlesFolderPath = configurationBuildDir
    .stringByAppendingPathComponent("AppToolkitCachedBundles" as String)
...
let appResourcesFolderPath = (targetBuildDir.stringByAppendingPathComponent(appExecutableDir as String) as NSString)
    .stringByAppendingPathComponent("AppToolkitRemoteResources" as String)
...
func retrieveRemoteBundlesManifest(apiToken: String, _ completion: ((bundles: [[NSObject: AnyObject]], error: NSError?) -> Void)?) {
    //print("Retrieving AppToolkit Remote Bundles Manifest...")
...
func copyCachedBundleToAppBundle(name:String, version:String) {
    ...
    // Make dir structure for [app]/AppToolkitRemoteResources/[bundle]/[version]
    ...
    // Copy contents of [cached]/[bundle]/[version]/* to [app]/AppToolkitRemoteResources/[bundle]/[version]/
    do {
...
retrieveRemoteBundlesManifest(apiToken, { (bundles, error) -> Void in
    if let error = error {
        ...
        print("Error caching AppToolkit remote resources due to error: \(error.domain) - \(error.code) - \(error.localizedDescription).")
        print("Verify that your network connection is established and working. Skipping for this build.")
        exit(EXIT_SUCCESS)
    } else {
        print("Caching AppToolkit remote resources to app bundle (for super-fast loads)")
